...
import os, zipfile, pythoncom, win32com.client
...
def word():
    ...
    in_p = os.path.abspath(os.path.join(UPLOAD_FOLDER, f.filename))
    out_p = in_p.replace('.docx', '.pdf')
    f.save(in_p)
    convert(in_p, out_p)
...
def excel():
    f = request.files['file']
    in_p = os.path.abspath(os.path.join(UPLOAD_FOLDER, f.filename))
    out_p = in_p.rsplit('.', 1)[0] + '.pdf'
    f.save(in_p)
    pythoncom.CoInitialize()
    xl = win32com.client.Dispatch("Excel.Application")
    wb = xl.Workbooks.Open(in_p)
...
# 9. SUPPRIMER/ORGANISER DES PAGES
...
def remove_pages():
    ...
    try:
        ...
        return "Erreur: Format de pages invalide (ex: 1,2,5)", 400
